...
///
/// @file string_view.h
/// @brief A readonly view over a contiguous sequence of characters, the storage of which is owned by another object.
//====================================================================================================================
...
public:
    ...
    constexpr basic_string_view(const_pointer p) : basic_string_view(p, p == nullptr ? 0 : Traits::length(p)) {}  // NOLINT (runtime/explicit)

    constexpr basic_string_view(std::string const& s) : basic_string_view(s.c_str(), s.size()) {}  // NOLINT (runtime/explicit)
    ...
    size_type find(basic_string_view v, size_type pos = 0) const noexcept { return find(v.data(), pos, v.size()); }
    ...
    size_type find(const_pointer s, size_type pos = 0) const { return find(s, pos, Traits::length(s)); }
