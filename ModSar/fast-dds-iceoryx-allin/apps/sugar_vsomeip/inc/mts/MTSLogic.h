...
namespace cmd
{
	...
	struct Stream
	{
		...
		virtual bool append(Object *object, unsigned int size);
		virtual bool pick(Object *object, unsigned int size);
	...
	struct QuickBinaryStream : public Stream {
		QuickBinaryStream(std::string &contents) : contents(contents)
		{
			_offset = 0;
            _size = 0;
		}
		virtual ~QuickBinaryStream() {}
		virtual bool append(const void *content, unsigned int size)
		{
			if (size == 0)
				return false;
...
#define PASRE_ARG(T, obj)                                                                                                       \
	typename remote::remove_pointer<typename remote::remove_const<typename remote::remove_ref<typename remote::charstostring<T>::type>::type>::type>::type obj; \
	remote::parseStream(obj, &ss);
...
public:
	...
	template <typename T>
	T *addComponent(const std::string &name,const std::function<T*(char*)> &callback)
	{
		auto i = _components.find(name);
		if (i == _components.end())
		{
			...
			if (ii.second)
			{
				return callback(&ii.first->second[0]);
			}
			return NULL;
		}
		else
