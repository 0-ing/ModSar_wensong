      ...
      //  printf("FrameHandler::push id:%d size:%d frameId:%d\n", id, (int)frameInfos[id].size(), info.frameId);
        ...
        { // 尝试唤醒消费者
          //  printf("需要唤醒其他\n");
            ...
            if (otherstub->has_func_repsonse()) {
                otherstub->response_func<std::function<void(const FrameInfo &)> >()(info);
            }
            otherstub->awake();
            return NON_BLOCK;
        }
        ...
        if (frameInfos[id].size())
        {
          ...
          //  printf("FrameHandler::pop id:%d size:%d frameInfo:%d\n", id, (int)frameInfos[id].size(), frameInfo.frameId);
            if (stub->has_func_repsonse()) {
                stub->response_func<std::function<void(const FrameInfo &)> >()(frameInfo);
            }
...
public:
     ...
     * 当队列为空时 不等待
     */
    void getFrameAsync(FrameInfo &info) // info 是值拷贝 buffer 不拷贝
    {
        std::function<void(const FrameInfo&)> action = [=](const FrameInfo &info)->void
        {
            printf("device:%d handle Frame id:%d\n", virtualID, info.frameId);
        };
        stub->async_expect_call(R(FrameHandler::pop), virtualID,action);
    ...
    void getFrame(FrameInfo &info)
    {
       // printf("getFrame..\n");
        ...
        stub->expect_call(R(FrameHandler::pop), virtualID,action);
...
void producer()
{
    ...
    if (stub2)
    {
        ...
        while (true)
        {
            ...
            info.frameId++;
            stub2->call(R(FrameHandler::push), 1, info);
