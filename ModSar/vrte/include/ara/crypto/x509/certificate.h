...
public:
    ...
    /// @error CryptoErrorDomain::kInsufficientCapacity  if <tt>(sn.empty() == false)</tt>, but its size
    ...
    /// @error CryptoErrorDomain::kInsufficientCapacity  if <tt>(id.empty() == false)</tt>, but its size is
    ...
    /// @error CryptoErrorDomain::kIncompleteArgState  if the @c hashCtx context is not initialized
    /// @threadsafety{Thread-safe}
