...
public:
    ...
    /// @error CryptoErrorDomain::kInvalidArgument  if the @c dn argument has incorrect format
    /// @error CryptoErrorDomain::kInvalidInputSize  if the @c dn argument has unsupported length (too large)
    /// @threadsafety{Thread-safe}
    ...
    /// @returns unique smart pointer for the created @c X509DN object
    /// @error CryptoErrorDomain::kInvalidArgument  if the @c dn argument cannot be parsed
    /// @error CryptoErrorDomain::kUnknownIdentifier  if the @c formatId argument has unknown value
    /// @threadsafety{Thread-safe}
    ...
    /// @error CryptoErrorDomain::kInvalidArgument  if the @c cert argument cannot be parsed
    /// @error CryptoErrorDomain::kUnknownIdentifier  if the @c formatId argument has unknown value
    /// @threadsafety{Thread-safe}
    ...
    /// @error CryptoErrorDomain::kInvalidArgument  if the @c certChain argument cannot be pre-parsed
    /// @error CryptoErrorDomain::kUnknownIdentifier  if the @c formatId argument has unknown value
    /// @threadsafety{Thread-safe}
    ...
    /// @param[in] certChain  DER/PEM-encoded certificate chain (in form of a single BLOB)
    /// @param[in] formatId  input format identifier (@c kFormatDefault means auto-detect)
    /// @error CryptoErrorDomain::kInsufficientCapacity  if the capacity of @c outcome vector is less
    ///                than actual number of certificates in the chain
    /// @error CryptoErrorDomain::kInvalidArgument  if the @c certChain argument cannot be parsed
    /// @error CryptoErrorDomain::kUnknownIdentifier  if the @c formatId argument has unknown value
    /// @threadsafety{Thread-safe}
    ...
    ///            in the input vector)
    /// @param[in] formatId  input format identifier (@c kFormatDefault means auto-detect)
    /// @error CryptoErrorDomain::kInsufficientCapacity  if capacity of the @c outcome vector is less
    ///                than number of elements in the @c certChain
    /// @error CryptoErrorDomain::kInvalidArgument  if an element of @c certChain argument cannot be parsed
    /// @error CryptoErrorDomain::kUnknownIdentifier  if the @c formatId argument has unknown value
    /// @threadsafety{Thread-safe}
    virtual ara::core::Result<void> ParseCertChain(ara::core::Vector<Certificate::Uptr>& outcome
    ...
    /// @error CryptoErrorDomain::kUnexpectedValue  if the provided BLOB is not a CRL/DeltaCRL
    /// @error CryptoErrorDomain::kRuntimeFault  if the CRL validation has failed
    /// @threadsafety{Thread-safe}
    ...
    /// @error CryptoErrorDomain::kInvalidArgument  if the provided certificate is invalid
    /// @error CryptoErrorDomain::kIncompatibleObject  if provided certificate has partial collision with
    ///                a matched CSR in the storage
    /// @error CryptoErrorDomain::kContentDuplication  if the provided certificate already exists
    ///                in the storage
    /// @error CryptoErrorDomain::kAccessViolation if the InstanceSpecifier points to a CertificateSlot,
    ...
    /// @error CryptoErrorDomain::kAccessViolation  if the method called by an application without
    ...
    /// @error CryptoErrorDomain::kInvalidArgument  if the provided certificate is invalid
    /// @error CryptoErrorDomain::kIncompatibleObject  if provided certificate doesn't belong to a CA
    /// @error CryptoErrorDomain::kAccessViolation  if the method called by an application without
    ...
    /// @error CryptoErrorDomain::kInvalidArgument  if the provided certificate is invalid
    /// @error CryptoErrorDomain::kIncompleteArgState  if the @c signer context is not initialized by a key
    /// @threadsafety{Thread-safe}
    ...
    /// @param[in] signer  an optional pointer to initialized signer context (if the request should be signed)
    /// @returns unique smart pointer to the created OCSP request
    /// @error CryptoErrorDomain::kInvalidArgument  if the provided certificates are invalid
    /// @error CryptoErrorDomain::kIncompleteArgState  if the @c signer context is not initialized by a key
    /// @threadsafety{Thread-safe}
    ...
    /// @error CryptoErrorDomain::kUnexpectedValue  if the provided BLOB @c response
    ...
    /// @error CryptoErrorDomain::kInvalidArgument  if the @c cert is invalid
    /// @error CryptoErrorDomain::kRuntimeFault  if the @c ocspResponse is invalid
    /// @threadsafety{Thread-safe}
    ...
    /// @returns a vector of unique smart pointers to found certificates; the vector is empty, if nothing is found
    /// @threadsafety{Thread-safe}
    virtual ara::core::Vector<Certificate::Uptrc> FindCertByDn(const X509DN& subjectDn
    ...
    /// @returns a vector of unique smart pointers to found certificates; the vector is empty, if nothing is found
    /// @threadsafety{Thread-safe} kUnknownIdentifier
    virtual ara::core::Vector<Certificate::Uptrc> FindCertByKeyIds(ReadOnlyMemRegion subjectKeyId
        , ara::core::Optional<ReadOnlyMemRegion> authorityKeyId
    ) noexcept = 0;

    /// @traceid{SWS_CRYPT_40633}@tracestatus{draft}
    /// @uptrace{RS_CRYPTO_02306}
    /// @brief Find a certificate by its serial number and issue DN.
    /// @param[in] sn  serial number of the target certificate
    /// @param[in] issuerDn  authority's Distinguished Names (DN)
    /// @returns the specified certificate or an error, if the certificate cannot be found
    /// @error CryptoErrorDomain::kUnknownIdentifier  if the specified certificate could not be found
    /// @threadsafety{Thread-safe}
    virtual ara::core::Result<Certificate::Uptrc> FindCertBySn(ReadOnlyMemRegion sn
        , const X509DN& issuerDn
    ) noexcept = 0;
    ...
    /// @error CryptoErrorDomain::kUnsupportedFormat  is returned in case the provided buffer does not contain the expected format
    /// @threadsafety{Thread-safe}
    ...
    /// @error CryptoErrorDomain::kInvalidArgument  if the provided certificate is invalid
    /// @error CryptoErrorDomain::kNoConnection  if the connection cannot be established
    /// @threadsafety{Thread-safe}
    ...
    /// @error CryptoErrorDomain::kInvalidArgument  if the provided certification request is invalid
    /// @error CryptoErrorDomain::kNoConnection  if connection to the CA cannot be established
    /// @threadsafety{Thread-safe}
    
    virtual ara::core::Result<void> SendRequest(const CertSignRequest& request
    ...
    ///       This method updates the @c Certificate::Status associated with the certificate.
    /// @error CryptoErrorDomain::kInvalidArgument  if the provided certificate is invalid
    /// @error CryptoErrorDomain::kNoConnection  if a connection to the OCSP responder cannot be established
    /// @threadsafety{Thread-safe}
    ...
    /// @error CryptoErrorDomain::kUnexpectedValue  if any of arguments has incorrect/unsupported value
    ///
    /// @threadsafety{Thread-safe}
    ...
    ///
    /// @error CryptoErrorDomain::kUnreservedResource  if the InstanceSpecifier is incorrect (the certificate cannot be found)
    /// @threadsafety{Thread-safe}
    ...
    /// @traceid{SWS_CRYPT_40914}@tracestatus{draft}
    /// @uptrace{RS_CRYPTO_02306}
    /// @brief Parse the custom X.509 extensions
    ///     This method parses the extensions of the provided certificate and calls the corresponding callbacks of the provided customExtensionsParser
    ///     for each parsed ASN.1 element.
    ///     If any call to one of the callbacks returns an error, 
    ///     the parsing stops and returns kRuntimeFault.
    ///     Parsing starts at the first extension of the certificate and parses all extensions of the certificate.
    ...
    /// @error CryptoErrorDomain::kRuntimeFault If parsing the extensions fails or calling one of the callback returns an error.
    /// @threadsafety{Thread-safe}
    virtual ara::core::Result<void> ParseCustomCertExtensions(
      const Certificate &cert, std::unique_ptr<X509CustomExtensionsParser> customExtensionsParser) const noexcept = 0;

    /// @traceid{SWS_CRYPT_40915}@tracestatus{draft}
    /// @uptrace{RS_CRYPTO_02306}
    /// @brief Parse the custom X.509 extensions
    ///     This method parses the extension identified by the provided oid of the provided certificate and 
    ///     calls the corresponding callbacks of the provided customExtensionsParser
    ///     for each parsed ASN.1 element.
    ///     If any call to one of the callbacks returns an error, 
    ///     the parsing stops and returns kRuntimeFault.
    ///     Only the sequence of the extension identified by the oid is parsed.
    ///
    /// @param[in] cert Certificate object to be parsed
    /// @param[in] customExtensionsParser Custom extensions parser that implements the callbacks
    /// @param[in] oid extension object identifier
    /// @error CryptoErrorDomain::kRuntimeFault If parsing the extensions fails or calling one of the callback returns an error.
    /// @error CryptoErrorDomain::kUnexpectedValue If the certificate doesn't contain an extension with the provided Oid.
    /// @threadsafety{Thread-safe}
    virtual ara::core::Result<void> ParseCustomCertExtensions(
      const Certificate &cert, std::unique_ptr<X509CustomExtensionsParser> customExtensionsParser,
      X509CustomExtensionsParser::Oid oid) const noexcept = 0;
