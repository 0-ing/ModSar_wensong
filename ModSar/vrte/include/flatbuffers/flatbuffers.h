...
class FlatBufferBuilder {
 public:
  ...
  /// @brief Read a scalar value in the buffer at a given offset.
  /// @return The value in the buffer.
  template<typename T>
  T ReadScalarAtOffset(uoffset_t offset) {
    return ReadScalar<T>(GetCurrentBufferPointer() + GetSize() - offset);
  }

  /// @brief Write a scalar value into the buffer at a given offset.
  template<typename T>
  void WriteScalarAtOffset(uoffset_t offset, T value) {
    WriteScalar<T>(GetCurrentBufferPointer() + GetSize() - offset, value);
  }
  //BOSCH Extension end
...
class Verifier FLATBUFFERS_FINAL_CLASS {
 public:
  ...
  uoffset_t VerifyOffset(size_t start) const {
    ...
    //BOSCH Extension start
    auto o = ReadScalar<soffset_t>(buf_ + start);
...
class Table {
 public:
  ...
  template<typename P> P GetPointer(voffset_t field) {
    ...
    //BOSCH Extension start
    return field_offset ? reinterpret_cast<P>(p + ReadScalar<soffset_t>(p))
                        : nullptr;
    //BOSCH Extension end
  }
...
volatile __attribute__((weak)) const char *flatbuffer_version_string =
  ...
  FLATBUFFERS_STRING(bosch) "-"
  FLATBUFFERS_STRING(FLATBUFFERS_VERSION_BOSCH_MAJOR) "."
  FLATBUFFERS_STRING(FLATBUFFERS_VERSION_BOSCH_MINOR);
  //BOSCH Extension end
