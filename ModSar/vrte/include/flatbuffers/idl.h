...
struct Value {
  ...
  // Related FB type.
  Type type;
  // Either the value itself or an offset, which points to the value.
  std::string constant;
  // Offset of the VTable entry.
  voffset_t offset;

  // Offset of the value inside of the FB
  uoffset_t offset_in_fb;
  // Children of the value
  std::vector<Value> children;
  // Key of the related JSON value
  std::string key;
  // Content string of the related JSON value
  std::string content;
  // Flag to indicate whether the content is a JSON path.
  // HINT: We need a flag to cleanly separate between JSON paths and strings,
  // which start with a '$'. 
  bool is_path;
  // Line no. in the related JSON file
  int line;
  // Col no. in the related JSON file
  int64_t col;
  //BOSCH Extension end
};
...
//BOSCH Extension start
/// @brief Parser Class for retrieving the JSON node of a given JsonPath.
class JsonPathParser {
public:
    /// @brief Parse a single path.
    /// @param path JsonPath, which needs to be resolved.
    /// @param value The related value node. Root shall be given as input,
    /// the resolved node is returned if resolving is successful.
    /// @param error An error message, if parsing is not successful.
    bool ParsePath(const std::string &path, const Value *&value, std::string &error);

private:
    /// @brief Internal class to represent the parser state.
    ...
    /// @brief Internal method to parse a struct componenet of a path.
    bool ParseStruct();

    /// @brief Internal method to parse an array filter expression of a path.
    bool ParseArrayFilter();

    /// @brief Internal method to parse an array index of a path.
    bool ParseArrayIndex();

    /// @brief The current position in the input path.
    const char* cursor_;
    /// @brief Current state of the parser.
    ParserState state_;
    /// @brief Matcher for regex calls.
    std::cmatch match_;
    /// @brief The current value (=node in the JSON tree).
    const Value *value_;
    /// @brief The error message of the parser.
    std::string error_;
};

/// @brief Helper class to construct the JSON tree during the parsing of an input json.
class JsonTree {
public:
  /// @brief Initialize the JSON tree.
  /// Will increase the depth by 1 and look up the root node.
  void StartTree(StructDef *root_struct_def_, SymbolTable<Type> &types);

  /// @brief Push a value to the current stack.
  /// Used to buffer object members before they get popped from the FB stack.
  void PushValue(Value &value);

  /// @brief Insert a value to the current stack.
  /// Used to buffer array children before they get popped from the FB stack.
  void InsertValue(Value &value);

  /// @brief Get the members of the object on the stack and move them into it.
  void SetObjectMembers(Value &value);

  /// @brief Go deeper into the tree, called before going through an array or object.
  void IncreaseDepth();

  /// @brief Finish the building of the tree by moving the stack into the root value.
  void EndTree();

  /// @brief Resolve all required paths in the tree.
  ///
  /// This function goes through all values with paths and parses through the JSON tree
  /// to find the correct address and replace the offset inside of the FB.
  ///
  FLATBUFFERS_CHECKED_ERROR ResolvePaths(FlatBufferBuilder &builder, const std::string &file_being_parsed, std::string &error);

private:
  /// @brief Deliver a message related to a given value in the JSON tree.
  ///
  /// This function will prepend the position of the value in the json file to the given message.
  /// HINT: The position is the first token after the actual JSON path. Getting the correct token position
  /// requires more effort, as the FB-internal json parser can't distinguish between string and json path
  /// while looking at the token, and cannot assign the token position to the related value easily.
  static void Message(const Value &value, const std::string &file_being_parsed, std::string &msg);

  /// @brief Storage for nodes
  ///
  /// Two-fold stack for remembering the values:
  /// One dimension is for the depth to identify parent/child relations,
  /// the other dimension is for all values of the current depth
  std::vector<std::vector<Value>> value_stack_;
  /// @brief List of values, which contain paths and need resolving.
  std::vector<Value> path_values_;
  /// @brief The root node of the JSON tree.
  Value root_;
};
//BOSCH Extension end
...
class Parser : public ParserState {
 public:
  ...
  //BOSCH Extension start
  // Similar to Error(), but the Error comes from a sub-component (e.g. JSON path parser).
  // This function will take the append the msg as-is and not prepend file line and col information.
  FLATBUFFERS_CHECKED_ERROR NestedError(const std::string &msg);
  //BOSCH Extension end

 private:
  void Message(const std::string &msg);
  //BOSCH Extension start
  // Similar to Message(), but will not prepend file line and col information.
  void NestedMessage(const std::string &msg);
  //BOSCH Extension end
  ...
  //BOSCH Extension start
  FLATBUFFERS_CHECKED_ERROR ParseRef(Value &val);
  //BOSCH Extension end
