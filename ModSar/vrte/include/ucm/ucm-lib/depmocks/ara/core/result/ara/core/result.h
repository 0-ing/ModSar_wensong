...
public:
    ...
    T const& Value() const &
    {
        T const* ptr = Variant::get<T>(&mData);
        if (ptr)
            return *ptr;
    ...
    E const& Error() const &
    {
        E const* ptr = Variant::get<E>(&mData);
        if (ptr)
            return *ptr;
