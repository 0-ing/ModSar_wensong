...
///
/// A SWP processing context is composed from the buffers for the 
/// Authentication tag and the data processing blocks.
///
/// At the moment the container is based on std::array. Should there be 
/// any performance considerations - it can be re-based on some linked 
/// list implementation.
///
...
///
class CtxCache final : virtual protected ucm::mock::Sequence
{
...
public:
    ...
    explicit CtxCache(lib::sec::Provider& ){}

    CtxCache(CtxCache     &&) = default;
    CtxCache(CtxCache const&) = delete;

    CtxCache& operator=(CtxCache     &&) = default;
    CtxCache& operator=(CtxCache const&) = delete;

    ~CtxCache() = default;

    std::shared_ptr<Context> v0102_acquire() noexcept
    { return pop<std::shared_ptr<Context>>(); }
