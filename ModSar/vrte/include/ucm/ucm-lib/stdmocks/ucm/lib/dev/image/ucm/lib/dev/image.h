...
public:
    ...
    auto& memslots() const noexcept { return mock::Sequence::pop<std::array<std::shared_ptr<MemSlot>, 4>&>(); }
    const auto& specs() const noexcept { return mock::Sequence::pop<Specs const&>(); }
    const auto& imgid() const noexcept { return mock::Sequence::pop<id_type const&>(); }
    const auto& mount_ro() const noexcept { return mock::Sequence::pop<bool const&>(); }
    const auto& redundancy() const noexcept { return mock::Sequence::pop<bool const&>(); }
    const auto& swcgroup() const noexcept { return mock::Sequence::pop<std::string const&>(); }

    const std::shared_ptr<MemSlot>& slot2write() { return mock::Sequence::pop<std::shared_ptr<MemSlot> const&>(); }

    void setslots(std::array<std::shared_ptr<MemSlot>, 4>&&) { }

    std::string printslots() const noexcept { return mock::Sequence::pop<std::string>(); }
