...
public:
    explicit TaskDeviceWrite(Tag&& tag, Callbacks&& callbacks,
                             std::unique_ptr<dev::Posix>&& device) noexcept;
        : Task(std::move(std::string("TaskDeviceWrite")),
...
private:
    ...
    ssize_t perform_write(const char*, size_t) noexcept { return pop<ssize_t>(); }
};
