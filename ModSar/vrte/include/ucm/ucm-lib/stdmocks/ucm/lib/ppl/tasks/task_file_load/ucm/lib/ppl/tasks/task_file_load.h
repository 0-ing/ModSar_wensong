...
public:
    explicit TaskFileLoad(Tag&& tag, Callbacks&& callbacks, const std::string& file)
    : Task(std::move(std::string("TaskFileLoad")),
...
private:
    ...
    bool load_token(Token*) noexcept { return pop<bool>(); }
};
