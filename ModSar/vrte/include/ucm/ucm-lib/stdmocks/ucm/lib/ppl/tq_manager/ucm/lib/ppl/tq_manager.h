...
public:
    explicit TQManager(std::shared_ptr<Bucket> bucket) noexcept
        : bucket_{bucket}
        , tq_incoming_{std::make_shared<TokenQueueMT>()}
        , tq_feedback_{std::make_shared<TokenQueueMT>()}
    {
    }
    ...
    void chain_next(std::shared_ptr<TQManager>) noexcept { pop<void>(); }
    void chain_prev(std::shared_ptr<TQManager>) noexcept { pop<void>(); }
    ...
    __attribute__((warn_unused_result))
    Token* inject_incoming(Token*) noexcept { return pop<Token*>(); }

    /// @brief inject the token into \ref tq_feedback_ queue
    __attribute__((warn_unused_result))
    Token* inject_feedback(Token*) noexcept { return pop<Token*>(); }

    /// @brief inject the token into \ref tq_next_ queue
    __attribute__((warn_unused_result))
    Token* forward_token(Token*) noexcept { return pop<Token*>(); }

    /// @brief inject the token into \ref tq_prev_ queue
    __attribute__((warn_unused_result))
    Token* feedback_token(Token*) noexcept { return pop<Token*>(); }
    ...
    bool has_next() const noexcept { return pop<bool>(); }
    bool has_prev() const noexcept { return pop<bool>(); }
...
{
    return pop<std::shared_ptr<Bucket>>();
}
...
inline Token* TQManager::token_acquire(const Tag&) noexcept
{
    return pop<Token*>();
}

inline Token* TQManager::token_release(Token*) noexcept
{
    return pop<Token*>();
}
