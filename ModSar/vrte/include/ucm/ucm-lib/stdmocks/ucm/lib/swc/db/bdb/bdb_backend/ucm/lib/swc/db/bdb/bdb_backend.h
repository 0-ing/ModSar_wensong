...
public:
    explicit BDB_Backend(std::string&& path)
        noexcept(std::is_nothrow_constructible<Backend>::value &&
                 std::is_nothrow_constructible<std::string>::value)
    : Backend(backend_name, std::move(path),
                  irecord_enable, hrecord_enable, frecord_enable, srecord_enable)
    {
    }
    ...
    virtual ~BDB_Backend() = default;

    virtual void startoff() override {return pop<void>();}
    virtual void shutdown() override {return pop<void>();}

    virtual std::unique_ptr<Handle> select_handle() override {
    			return pop<std::unique_ptr<Handle>>();}
    virtual std::unique_ptr<Handle> update_handle() override {
    			return pop<std::unique_ptr<Handle>>();}

private:
    void ensure_directory() {return pop<void>();}
};
