...
public:
    ...
    explicit TmpFile(off_t size, const std::string& templ,
                     bool autodel = true) noexcept(false)
             : TmpFile(size, templ.c_str(), autodel)
    {}
    virtual ~TmpFile() = default;

    static std::string make(const std::string& templ) noexcept(false) { return pop<std::string>();}
    static std::string
    make(off_t size, const std::string& templ) noexcept(false) { return pop<std::string>();}

    off_t size_ = 0;

private:
    std::string maketemp(char* path) noexcept(false) { return pop<std::string>();}
    std::string maketemp(const std::string& path) noexcept(false) { return pop<std::string>();}
};
