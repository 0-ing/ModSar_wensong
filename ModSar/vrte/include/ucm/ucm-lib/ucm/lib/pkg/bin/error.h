...
namespace internal {
constexpr UCM_LIB_PKG_BIN_ErrorDomain ucm_lib_pkg_bin_error_domain;
}   // namespace internal

constexpr const ErrorDomain& UCM_LIB_PKG_BIN_ErrorDomain_Get() noexcept
{
    return internal::ucm_lib_pkg_bin_error_domain;
}

constexpr ErrorCode make_error_code(ErrorValue ev) noexcept
{
    return ErrorCode(static_cast<err::value_type>(ev),
                     UCM_LIB_PKG_BIN_ErrorDomain_Get());
}
