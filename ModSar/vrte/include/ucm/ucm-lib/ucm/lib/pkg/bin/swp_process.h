...
{
#if defined(GOOGLE_TEST)
    friend class Test_ucm_lib_pkg_bin_swp_process;
    FRIEND_TEST( Test_ucm_lib_pkg_bin_swp_process, check_outdir_stat_failure);
    FRIEND_TEST( Test_ucm_lib_pkg_bin_swp_process, check_outdir_not_a_directory);
    FRIEND_TEST( Test_ucm_lib_pkg_bin_swp_process, check_outdir_success);
    FRIEND_TEST( Test_ucm_lib_pkg_bin_swp_process, check_swpfile_stat_failure);
    FRIEND_TEST( Test_ucm_lib_pkg_bin_swp_process, check_swpfile_success);
#endif

protected:
    /// @brief Create a SWP processor to extract to directory.
    explicit SWP_Process(const std::string& outdir,
