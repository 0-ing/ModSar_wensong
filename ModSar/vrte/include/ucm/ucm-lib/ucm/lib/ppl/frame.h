...
public:
    ...
    std::size_t vptr_append(const void *data, std::size_t size);
...
inline void Frame::vsize_reset(std::size_t size) noexcept
{
    vsize_ = size;
}

inline void Frame::cptr_reset(const void *data, std::size_t size) noexcept
{
    cptr_ = data;
    csize_ = size;
}
...
{
    return vptr_ != nullptr ? false : true;
}
