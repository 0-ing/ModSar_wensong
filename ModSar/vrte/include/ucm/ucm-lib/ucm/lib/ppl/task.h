...
public:
    ...
    explicit Task(std::string&&, Tag&&, Callbacks&&) noexcept;
...
protected:
    ...
    void forward_signal(InOutArgs&, Signal&&) noexcept;

    void feedback_signal(InOutArgs&, Signal&&) noexcept;

    void broadcast_signal(InOutArgs& args, Signal&& sig) noexcept;
...
{
    return complete_ and unit_done_;
}
...
{
    return token_create_cb_();
}

inline Token* Task::token_release(Token* token) noexcept
{
    return token_release_cb_(token);
}

inline Token* Task::token_acquire(const Tag& tag) noexcept
{
    return token_acquire_cb_(tag);
}

inline Token* Task::input_token(Task::InOutArgs& args) noexcept
{
    return args.input_token_;
}

inline Token* Task::forward_token(Task::InOutArgs& args, Token* token) noexcept
{
    args.forward_token_ = token;
    return nullptr;
}

inline Token* Task::feedback_token(Task::InOutArgs& args, Token* token) noexcept
{
    args.feedback_token_ = token;
    return nullptr;
}
