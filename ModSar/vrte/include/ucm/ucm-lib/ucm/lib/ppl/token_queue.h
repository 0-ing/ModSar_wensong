...
public:
    ...
    virtual Token* acquire() noexcept;

    __attribute__((warn_unused_result))
    virtual Token* release(Token*) noexcept;

    virtual std::size_t level() const noexcept;
...
inline std::size_t TokenQueue::level() const noexcept
{
    return level_;
}
